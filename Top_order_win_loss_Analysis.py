...
# Filter only top-order batsmen
top3_df = df[df['batting_position'].isin(top3_positions)]


# Group by match_id and match_result, sum the runs
match_top3_runs = top3_df.groupby(['match_id', 'match_result'], as_index=False)['runs'].sum()

# Separate into wins and losses
wins = match_top3_runs[match_top3_runs['match_result'] == 'Win']['runs']
losses = match_top3_runs[match_top3_runs['match_result'] == 'Loss']['runs']

# Descriptive statistics
print("=== top3‑order runs per match ===")
...
alpha = 0.05
if p_value < alpha:
    print("\n→ Reject H₀: There is a statistically significant difference in top‑order runs between wins and losses.")
else:
    print("\n→ Fail to reject H₀: No statistically significant difference in top‑order runs between wins and losses.")
...
if abs_d < 0.2:
    ...
    cricket_meaning = "Virtually no dependency on the top Order runs between wins and losses."
elif abs_d < 0.5:
    interpretation = "Small effect"
    cricket_meaning = "Mild dependency on the top order runs between wins and losses."
elif abs_d < 0.8:
    interpretation = "Medium effect"
    cricket_meaning = "Moderate dependency on the top Order runs between wins and losses."
elif abs_d < 1.2:
    interpretation = "Large effect"
    cricket_meaning = "Strong dependency on the top Order runs between wins and losses."
else:
    interpretation = "Very Large effect"
    cricket_meaning = "Extreme dependency on the top Order runs between wins and losses."
